...
/* ----------------------------------------------------------------------
   ...
   RS 2023: changes to match with current lammps (nmc version new_Sept2022)
   => do not access protected flags but use setter methods.
------------------------------------------------------------------------- */
...
void PairNEQUIP::init_style(){
  ...
  // RS change to enable_full()
  // neighbor->requests[irequest]->half = 0;
  // neighbor->requests[irequest]->full = 1;
  neighbor->requests[irequest]->enable_full();

  // TODO: probably also
  // RS this is zero by default. only to switch it on (ghost = 1) we would have to call enable_ghost()
