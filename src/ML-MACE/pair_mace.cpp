...
#include "neighbor.h"
#include "update.h" // RS needed to access lammps units
...
void PairMACE::compute(int eflag, int vflag)
{
  ...
  if (eflag_global) {
    ...
    eng_vdwl += energy.item<double>()*econvert; // RS convert unit
  }
  ...
  for (int ii=0; ii<n_nodes; ++ii) {
    int i = list->ilist[ii];
    atom->f[i][0] += forces[i][0].item<double>()*econvert;
    atom->f[i][1] += forces[i][1].item<double>()*econvert;
    atom->f[i][2] += forces[i][2].item<double>()*econvert;
  }
  ...
  if (eflag_atom) {
    ...
    for (int ii=0; ii<list->inum; ++ii) {
      int i = list->ilist[ii];
      eatom[i] = node_energy[i].item<double>() * econvert;
    }
  }
  ...
  if (vflag_global) {
    ...
    virial[0] += vir[0][0][0].item<double>()*econvert;
    virial[1] += vir[0][1][1].item<double>()*econvert;
    virial[2] += vir[0][2][2].item<double>()*econvert;
    virial[3] += 0.5*(vir[0][1][0].item<double>() + vir[0][0][1].item<double>())*econvert;
    virial[4] += 0.5*(vir[0][2][0].item<double>() + vir[0][0][2].item<double>())*econvert;
    virial[5] += 0.5*(vir[0][2][1].item<double>() + vir[0][1][2].item<double>())*econvert;
  }
...
void PairMACE::coeff(int narg, char **arg)
{
  ...
  // RS check lammps unit system, allowed are metal and real. for metal all is fine, for real we need
  //     to convert vom eV -> kcal/mol
  if (strcmp("metal", update->unit_style) == 0) {
    std::cout << "Using metal units (econvert = 1.0)";
    econvert = 1.0;
  }
  else if (strcmp("real", update->unit_style) == 0) {
    std::cout << "Using real units (econvert = 23.0609)";
    econvert = 23.0609;
  }
  else {
    error->all(FLERR, "MACE Potential must use either metal or real units");
  // RS end
  }
